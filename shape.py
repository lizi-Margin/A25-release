...
# 输出视频路径（可选）
near_ir_output_path = "./video/s_output_rgb_smoked3.mp4"
far_ir_output_path = "./video/s_output_tr_smoked3.mp4"
...
# 逐帧处理视频
while near_ir_cap.isOpened():
    ret1, frame_1 = near_ir_cap.read()
    if not ret1:
        print("End of video1 reached.")
        break
    rgb_resized = cv2.resize(frame_1, (704, 419), interpolation=cv2.INTER_AREA)
    rgb_cropped = rgb_resized[:, 32:672]
    # 显示处理后的帧（可选）
    cv2.imshow("Near IR Processed", rgb_cropped)
    # 保存
    near_ir_out.write(rgb_cropped)
    # 按 'q' 键退出
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break


while far_ir_cap.isOpened():
    ret2, frame_2 = far_ir_cap.read()
    if not ret2:
        print("End of video2 reached.")
        break
    ir_cropped = frame_2[66:485, :]
    # 显示处理后的帧（可选）
    cv2.imshow("Far IR Processed", ir_cropped)
    # 保存
    far_ir_out.write(ir_cropped)
    # 按 'q' 键退出
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
